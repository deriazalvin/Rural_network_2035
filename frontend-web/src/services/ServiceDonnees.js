    ...
    export class ServiceDonnees {
      ...
      async request(path, options = {}) {
        ...
        if (!res.ok) {
          // Détection 401 → déconnexion globale
          if (res.status === 401) {
            window.dispatchEvent(new CustomEvent('rn-auth-expired', { detail: { message: 'Session expirée' } }));
          }
      ...
      // Optimisations
      obtenirHistoriqueOptimisations() { return this.request('/optimisations/historique'); }
      viderHistoriqueOptimisations() { return this.request('/optimisations/historique', { method: 'DELETE' }); }
